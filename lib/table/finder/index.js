    ...
    function Finder(props) {
        ...
        _this.getChilds = function (lists, notEnum, parentIndex) {
            return lists.map(function (item, index) {
                ...
                switch (item.type) {
                    case 'text':
                        return _react2.default.createElement(_fitInput2.default, _extends({ key: index,
                            style: itemStyle
                        }, item.props, {
                    ...
                    case 'time':
                        return _react2.default.createElement(_fitTimepicker2.default, _extends({ key: index,
                            style: itemStyle,
                    ...
                    case 'date':
                        return _react2.default.createElement(_fitDatepicker.DateInput, _extends({ key: index,
                            style: itemStyle,
                            input: { label: notEnum ? null : item.label }
                        }, item.props, {
                            defaultValue: item.value || item.defaultValue,
        ...
        value: function handleSearch() {
            ...
            this.state.opts.map(function (item) {
                    ...
                    item.enum.map(function (elItem) {
                        if (elItem.key === (item.value || item.defaultValue) && (elItem.value || elItem.defaultValue)) {
                            if (elItem.beforeSend) {
                                var newParams = elItem.beforeSend(elItem.key, elItem.value || elItem.defaultValue);
                ...
                if (item.value || item.defaultValue) {
                    if (item.beforeSend) {
                        var newParams = item.beforeSend(item.key, item.value || item.defaultValue);
                        params = _jquery2.default.extend(params, newParams);
                    } else {
